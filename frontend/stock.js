// Stock Bot page. Reads /api/stock/state and renders status, settings, open
// positions, an equity / cumulative-realized chart, closed trades, and the
// cron run log. Writes go through /api/stock/settings and /api/stock/run.
// A simplified sibling of ai.js - plain stock, no options/tiers/puts.
...
function signalCell(sig) {
  if (!sig) return "—";
  const z = sig.z != null ? `z ${Number(sig.z).toFixed(2)}` : "";
  const dd = sig.drawdown_pct != null ? `dd ${Number(sig.drawdown_pct).toFixed(0)}%` : "";
  const rsi = sig.rsi != null ? `RSI ${Number(sig.rsi).toFixed(0)}` : "";
  return `<span class="ai-sub">${esc([z, dd, rsi].filter(Boolean).join(" · "))}</span>`;
}
...
function renderChart(series) {
  ...
  if (!series || series.length === 0) {
    el("stk-pnl-empty").hidden = false;
    host.style.display = "none";
    return;
  }
  el("stk-pnl-empty").hidden = true;
...
function renderState(d) {
  ...
  el("stk-enabled").checked = enabled;
  el("stk-enabled-note").textContent = enabled
  ...
  el("stk-config-warn").hidden = !!d.configured;

  settingsCache = d.settings || {};
  for (const inp of document.querySelectorAll("#stk-settings [data-set]")) {
  ...
  el("stk-tiles").innerHTML = tiles.map(([k, v]) =>
    `<div class="ai-tile"><div class="ai-tile-k">${k}</div><div class="ai-tile-v">${v}</div></div>`).join("");

  // Open positions
  const ob = el("stk-open-table").querySelector("tbody");
  ob.innerHTML = "";
  el("stk-open-empty").hidden = d.open_trades.length > 0;
  for (const t of d.open_trades) {
    ...
    tr.innerHTML = `
      ...
      <td>${held}</td>
      <td class="ai-sub">${t.tp?.limit_price != null ? "$" + t.tp.limit_price : "—"}</td>`;
    ob.appendChild(tr);
  }

  // Stats line
  const s = d.stats;
  el("stk-stats").innerHTML = s.closed_count
  ...
  // Closed trades
  const cb = el("stk-closed-table").querySelector("tbody");
  cb.innerHTML = "";
  el("stk-closed-empty").hidden = d.closed_trades.length > 0;
  ...
  // Run log
  el("stk-runlog").innerHTML = (d.run_log || []).map((r) => {
    const lines = [];
...
async function load() {
  try {
    const r = await fetch("/api/stock/state");
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
    renderState(await r.json());
  } catch (e) {
    el("stk-runlog").innerHTML = `<div class="ai-rl-err">Failed to load state: ${esc(e.message)}</div>`;
  }
}

async function postSettings(patch) {
  const r = await fetch("/api/stock/settings", {
    method: "POST",
...
el("stk-enabled").addEventListener("change", async (e) => {
  try {
    await postSettings({ enabled: e.target.checked });
    await load();
  } catch (err) {
    el("stk-save-status").textContent = `Failed: ${err.message}`;
    e.target.checked = !e.target.checked;
  }
});

el("stk-save-settings").addEventListener("click", async () => {
  const patch = {};
  for (const inp of document.querySelectorAll("#stk-settings [data-set]")) {
    if (inp.value !== "") patch[inp.dataset.set] = Number(inp.value);
  }
  const status = el("stk-save-status");
...
el("stk-run-now").addEventListener("click", async () => {
  const btn = el("stk-run-now");
  btn.disabled = true;
  btn.textContent = "Running…";
  try {
    const r = await fetch("/api/stock/run", { method: "POST" });
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
    await load();
  } catch (err) {
    el("stk-save-status").textContent = `Run failed: ${err.message}`;
  } finally {
    btn.disabled = false;
