"""S&P 500 universe scanned by the long-stock bot (stock_trader).

503 tickers, the universe the thesis-2 capitulation strategy was validated
on. Symbols are normalized to yfinance form (class shares BRK.B -> BRK-B).
Distinct from sp500_tickers.SP500_TICKERS (a 185-name large-cap subset used
by the discovery scanner).

Survivorship note: this is current index membership; names that left the
S&P 500 are absent. The bot's live results carry the same mild upward bias
as the backtest - see research/thesis2/THESIS.md Step 19.
"""
